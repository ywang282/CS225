...
class KDTree
{
    ...
    private:
       ...
       void swap(vector< Point<Dim> > & points,int lo, int hi);

         int partition(vector< Point<Dim> > & points,int lo, int hi,int pivot,int curDim);
       
        void quickselect(vector< Point<Dim> > & points,int lo, int hi,int mid,int curDim);

        void buildTree(vector< Point<Dim> > & points,int lo, int hi,int curDim);

      Point<Dim> findNearestNeighbor(const Point<Dim> & query,int lo,int hi,int curDim) const;
