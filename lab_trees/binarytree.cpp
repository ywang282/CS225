...
bool BinaryTree<T>::isOrdered(Node * subRoot) const
{
	bool out1=false;
    	...
    	out1=isOrdered(subRoot->left);
    	out2=isOrdered(subRoot->right);
    	if(out1&&out2){
    		if(subRoot->left!=NULL){
    			out1=subRoot->elem>=subRoot->left->elem;
    		}
    		if(subRoot->right!=NULL){
    			out2=subRoot->elem<=subRoot->right->elem;
    		}
    	}
...
template <typename T>
void BinaryTree<T>::printPaths(Node * subRoot,int size, T arr[]) const
{
	if(subRoot==NULL){
        return;
    }
    ...
    arr[size]=subRoot->elem;
    size++;
    if(subRoot->left!=NULL){
        printPaths(subRoot->left,size,arr);
    }
   
       
    if(subRoot->right!=NULL){
        printPaths(subRoot->right,size,arr);
    }
