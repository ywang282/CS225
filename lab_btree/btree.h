...
size_t insertion_idx(const std::vector< T >& elements, const C& val,size_t lo,size_t hi)
{
   ...
   if(hi-lo==1){
       if(elements[lo]>val||elements[lo]==val){
        return lo;
       }
       else {
            if(elements[hi]<val){
   ...
   if(elements[mid]==val){
        ...
        return insertion_idx(elements, val,lo,mid);
   }
   else {
        return insertion_idx(elements, val,mid,hi);
   }
...
size_t insertion_idx(const std::vector< T >& elements, const C& val)
{
   if(elements.size()==0){
    ...
    size_t length=elements.size()-1;
    return insertion_idx(elements,val,0,length);
}
