...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
   ...
   auto itfirst=dict.find(first);
   auto itsecond=dict.find(second);
   if(itfirst!=dict.end()&&itsecond!=dict.end()){
	   if((itfirst->second).size()!=(itsecond->second).size()){
	   	out=false;
	   }
	   else{
	   	for(size_t i=0;i<(itfirst->second).size();i++){
	   		if(itfirst->second[i].compare(itsecond->second[i])!=0){
	   			out=false;
	   		}
	   	}
